            ...
            with get_db_session() as db:
                defect = Defect(
                    ...
                    evidence_json={
                        ...
                        "iteration": request.review_comment if request.review_comment and request.review_comment.startswith("v") else "",
                        "issue_type": "代码对比",
                        "requirement_source": "需求-代码对比分析",
                        "repro_steps": steps_text,
                        "actual_result": finding.get("analysis", ""),
                        "expected_result": suggestion.get("expected", ""),
                        "evidence_text": f"Finding 类型: {finding.get('type')}; 置信度: {finding.get('confidence')}",
                        "code_refs": f"{evidence.get('file', 'N/A')}:{evidence.get('line', 'N/A')}",
                        "api_refs": "",
                        "suggested_fix": suggestion.get("suggested_fix") or suggestion.get("suggestion") or "",
